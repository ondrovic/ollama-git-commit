...
export class OllamaService {
  ...
  parseResponse(response: string, verbose: boolean, model: string, host: string): string {
    try {
      ...
      if (typeof error === 'object' && error && 'message' in error) {
        if ((error as { message: string }).message === 'Model not found') {
          throw error;
        }
      }
      ...
      Logger.error(`Failed to parse response: ${(error as { message: string }).message}`);
      if (verbose) {
  ...
  async pullModel(host: string, model: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(`${host}/api/pull`, {
        ...
        signal: AbortSignal.timeout(timeouts.modelPull), // Use config timeout instead of hardcoded
      });

      return response.ok;
    } catch (error: unknown) {
      if (typeof error === 'object' && error && 'message' in error) {
        Logger.error(`Failed to pull model: ${(error as { message: string }).message}`);
      } else {
        Logger.error(`Failed to pull model: ${String(error)}`);
      }
      return false;
    }
  }
