...
function parseCliArgs(): CliArgs {
  try {
    ...
    const commandsThatDontNeedDirectory = [
      'help',
      'test',
      'list-models',
      'test-simple',
...
async function showConfigurationDebug(): Promise<void> {
  try {
    ...
    Logger.error('Failed to show debug information:', error.message);
    process.exit(1);
  }
}
...
async function testInteractivePrompt(): Promise<void> {
  try {
    ...
    Logger.error('Interactive prompt test failed:', error.message);
    if (Logger.isDebug()) {
  ...
  try {
    ...
    if (args['debug-clipboard']) {
      ...
      try {
        ...
        Logger.error('Failed to test clipboard:', error.message);
        if (Logger.isDebug()) {
          console.error(error.stack);
        }
      }
    ...
    if (error.message.includes('connect') || error.message.includes('Ollama')) {
      console.log('');
...
//     ollama-commit -m codellama       # Override model
//     ollama-commit --config-init      # Create default config file
//     echo "OLLAMA_HOST=http://remote:11434" > .env && ollama-commit
